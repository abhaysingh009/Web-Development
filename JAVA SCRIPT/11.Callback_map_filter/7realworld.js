...
// show items only where price is greater than 100;
// callback=(num)=>num.price>100
// const temp=products.filter(callback)
// console.log(temp)

// show only name price category
// const pro=products.filter((inst)=>inst.inStock)
// const temp2=pro.map((goods)=>({name:goods.name, price:goods.price,category:goods.category}));
// console.log(pro)
// or in single pass
const pro=products.filter((inst)=>inst.inStock).map((goods)=>({name:goods.name, price:goods.price,category:goods.category}));// add items that are in stock
console.log(pro)
  
// alt+a+s --> to word wrap