...
for(const prod of products){
    ...
    brand.textContent=`Brand: ${prod.brand}`;

    const rating=document.createElement('p')
    rating.textContent=`Rating: ${prod.rating}`;

    const discount=document.createElement('p')
    discount.textContent=`Discount: ${prod.discount}%`;

    const inStock=document.createElement('p')
    // inStock.textContent=prod.inStock;
    if(prod.inStock)inStock.textContent='Available'
    else inStock.textContent='Not Available'
    ...
    // horizontal line after image
    const line=document.createElement('hr')

    card.append(image,line,title,price,category,brand,rating,discount,inStock);
    arr.push(card)
}
